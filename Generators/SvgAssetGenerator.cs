...
public sealed class SvgAssetGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(
            ...
            static (productionContext, collectedDeclarations) =>
                GenerateRegistries(productionContext, collectedDeclarations!));
    }
    ...
    private static void GenerateRegistries(
        SourceProductionContext context,
        ImmutableArray<AssetDeclaration?> declarations)
    {
        foreach (var group in declarations
                     .Where(static declaration => declaration is { Valid: true })
                     .Select(static declaration => declaration!)
                     .GroupBy(static declaration => new
                     {
                         declaration.Namespace,
                         declaration.TypeName,
                         declaration.TypeAccessibility,
                         declaration.TypeIsStatic,
                     }))
        {
            var entries = new List<(string Name, string Expression)>();
            foreach (var declaration in group)
            {
                for (var index = 0; index < declaration.Paths.Length; index++)
                {
                    var path = NormalizePath(declaration.Paths[index]);
                    var fileName = path.Substring(path.LastIndexOf('/') + 1);
                    var extensionIndex = fileName.LastIndexOf('.');
                    var name = extensionIndex > 0 ? fileName.Substring(0, extensionIndex) : fileName;
                    var expression = declaration.Paths.Length == 1
                        ? declaration.PropertyName
                        : declaration.PropertyName + "[" + index + "]";
                    entries.Add((name, expression));
                }
            }

            var duplicate = entries.GroupBy(static entry => entry.Name, StringComparer.OrdinalIgnoreCase)
                .FirstOrDefault(static entry => entry.Count() > 1);
            ...
            {
                source.Append("namespace ").Append(group.Key.Namespace).AppendLine(";");
                source.AppendLine();
            }
            ...
            source.AppendLine("        new global::System.Collections.Generic.Dictionary<string, global::HyprNetShell.Rendering.SvgAsset>(global::System.StringComparer.OrdinalIgnoreCase)");
            source.AppendLine("        {");
            foreach (var entry in entries.OrderBy(static entry => entry.Name, StringComparer.OrdinalIgnoreCase))
            {
                source.Append("            [")
                    .Append(SymbolDisplay.FormatLiteral(entry.Name, true))
                    .Append("] = ").Append(entry.Expression).AppendLine(",");
            }
            source.AppendLine("        };");
            source.AppendLine();
            source.AppendLine("    public static global::System.Collections.Generic.IReadOnlyDictionary<string, global::HyprNetShell.Rendering.SvgAsset> ByName => __svgAssetsByName;");
            source.AppendLine("}");

            context.AddSource(
                group.Key.TypeName + ".SvgAssetRegistry.g.cs",
                SourceText.From(source.ToString(), Encoding.UTF8));
        }
    }
